...
if (!text) return conn.reply(m.chat, `🚩 Ingresa el texto de lo que quieres buscar en pinterest`, m, rcanal)
  
try {
let api = await fetch(`https://bk9.fun/pinterest/search?q=${text}`)
let json = await api.json()
if (!json || !json.BK9 || !json.BK9.length) return conn.reply(m.chat, `🚩 No se encontraron resultados para ${text}.`, m, rcanal)
