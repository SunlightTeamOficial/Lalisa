...
return m.reply("Este comando solo puede ser usado en la bot principal! wa.me/" + global.conn.user.jid.split`@`[0x0] + "?text=" + usedPrefix + "serbot")
}
...
async function serbot() {
    ...
    async function connectionUpdate(update) {
      ...
      if (qr) {
        let txt = `> ❀ » Escanea este QR para ser en un Sub Bot\n`
            txt += `> ❀ » Pasos para escanear\n`
            txt += `> ❀ » 1 : Haga click en los 3 puntos\n`
            txt += `> ❀ » 2 : Toque dispositivos vinculados\n`
            txt += `> ❀ » 3 : Escanea este QR y listo.`
